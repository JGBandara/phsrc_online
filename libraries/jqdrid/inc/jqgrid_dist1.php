 ...
 */
 ...
 error_reporting(E_ALL & ~E_NOTICE & ~E_DEPRECATED);                                                                                                                                                                                                                                                                                                                        class jqgrid{
		...
		function jqgrid($V874bee68 = null,$db)	{
			...
			$arrOptions["cellEdit"] 		= false;
			$arrOptions["add_options"] 		= array("closeAfterAdd"=>false);
			$arrOptions["edit_options"] 	= array("closeAfterEdit"=>false);
...
private function to_sql($V06e3d36f, $oper, $V3a6d0284) 	{
	if($oper=='bw' || $oper=='bn') 
		return "'" . addslashes($V3a6d0284) . "%'";else if ($oper=='ew' || $oper=='en') return "'%" . addcslashes($V3a6d0284) . "'";else if ($oper=='cn' || $oper=='nc') return "'%" . addslashes($V3a6d0284) . "%'";else return "'" . addslashes($V3a6d0284) . "'";
}	 
...
function get_dropdown_values($SQL)	{
	...
	if ($this->db) {
		...
		foreach($row as $V3a2d7564)
			...
			while($V3a2d7564 = mysql_fetch_array($result,MYSQL_ASSOC)) {
				 $V341be97d[] = $V3a2d7564["k"].":".$V3a2d7564["v"];
			}
	}
		...
		if (!$this->options["colNames"]) 
			...
			if (!isset($_REQUEST['_search'])) 
				...
				if (!$V39b7b22d) { 
					...
					$V6148bbf9 .= " AND ".$V39b7b22d;switch ($V2acdba16) {
					...
					case "ni": $V6148bbf9 .= " NOT IN (".$V930b8af8.")";
						break;case "bw": 
					default: $V930b8af8 .= "%";$V6148bbf9 .= " LIKE '".$V930b8af8."'";break;
				}
			}
		}  
	...
	if (isset($_GET['page'])) { 
		...
		$sidx = str_replace("::",".",$sidx);  
			...
			if ($this->db) { 
				...
				//$Vf1965a85 = $result->FetchRow();$Vf1965a85 = $Vf1965a85[0];
				$Vf1965a85 = mysql_fetch_array($result,MYSQL_ASSOC);
			}else { 
			...
			if ($Vea2b2676<0) 
				...
				if ($this->db) { 
					...
					$rows = array();
					while($Vf1965a85 = mysql_fetch_array($result,MYSQL_ASSOC)) 
						$rows[] = $Vf1965a85;
				}else { 
					$rows = array();
					...
					foreach ($rows as $Vf1965a85)	 {  
						...
						if ($this->actions["rowactions"] !== false) {  $V8fa14cdd = false;$V7238ac6d = false;foreach($this->options["colModel"] as &$V4a8a08f0) { if ($V4a8a08f0["name"] == "act") { 
						$V7238ac6d = &$V4a8a08f0;
						} 
