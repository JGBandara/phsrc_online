...
left Join tbl_owner ON reg_ins_comm.ownership_type = tbl_owner.ownership_id
		...
		while($row=mysqli_fetch_array($result))
		{
			$val1['facility'] 		= $row['facility'];
			$val1['value'] 	= $row['value'];
			$val1['description'] 	= $row['description'];
			$arrDetail1[] = $val1;
		}
		...
		{
			$val2['Name'] 		= $row['Name'];
			$val2['Qulification'] 	= $row['Qulification'];
			$val2['institute'] 	= $row['institute'];
			$val2['country'] 		= $row['country'];
			$val2['post_gradu'] 	= $row['post_gradu'];
			$val2['speciality'] 	= $row['speciality'];
			$val2['Register_id'] 	= $row['Register_id'];
			$arrDetail2[] = $val2;
		}
		...
		while($row=mysqli_fetch_array($result))
		{
			$val3['position_id'] 			= $row['position_id'];
			$val3['name'] 		= $row['name'];
			$val3['contact_detail'] 	= $row['contact_detail'];
			$val3['info'] 		= $row['info'];
