...
$response = [];
$autoNoType = "";   
//$txtNoBed = isset($_REQUEST['txtNoBed'])?trim($_REQUEST['txtNoBed']):null;
$txtNoBed = 0;
$txtNoRoom = isset($_REQUEST['txtNoRoom'])?trim($_REQUEST['txtNoRoom']):null;
$txtNoWard = isset($_REQUEST['txtNoWard'])?trim($_REQUEST['txtNoWard']):null;
  ...
  try{
    ...
    if($row = mysqli_fetch_row($result)){
      ...
      $sql="update `institute_facility`
            set
					ins_no_of_bed        	='$txtNoBed',
					ins_no_of_room    		='$txtNoRoom',
					ins_no_of_ward      	='$txtNoWard',
      ...
      $sql="insert into `institute_facility`
            ( ins_faci_institute_id,ins_no_of_bed,ins_no_of_room,ins_no_of_ward,ins_radio_service,ins_waste_disposal,ins_inst_dress,ins_emergency_kit, ins_info_company_id, ins_info_created_by, ins_info_created_on)
              values              ('$id','$txtNoBed','$txtNoRoom','$txtNoWard','$cboAtomicEnergy','$txtclinicalDis','$txtInsDress','$cboEmgKit', '1', '1', '". time()."')";
