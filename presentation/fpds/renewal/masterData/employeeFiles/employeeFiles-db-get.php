...
) and dms_file_category.dfc_file_group_id=1 and dms_file_category.dfc_status=1 and dms_file_category.dfc_is_deleted=0";
  $result = $db->singleQuery($sql);
  $arr = [];
  while($row = mysqli_fetch_assoc($result)){
    $arr[] = $row;
...
reg_no
FROM
institute_registration where ins_is_deleted='0' and ins_type_id='7'  and institute_reg_id='$id'
