...
try{
  ...
  $conditionStr = "1=1 and dfg_company_id='$userCompanyId'";
  $sql = "select *
          from dms_file_group
          where ".$conditionStr."
          order by dfg_id asc";
  ...
  $fieldNameStr = " dfg_id, dfg_name, dfg_remarks, ifnull(stat_name,'') as `status`, if(dfg_is_deleted='1','Yes','No') as `is deleted`, ifnull(syc_name,'') as `company`, ifnull(cu.syu_full_name,'') as `created by`, from_unixtime(dfg_created_on,'%Y-%m-%d') as `created on`, ifnull(mu.syu_full_name,'') as `modified by`, from_unixtime(dfg_last_modified_on,'%Y-%m-%d') as `last modified on`, ifnull(du.syu_full_name,'') as `deleted by`, from_unixtime(dfg_deleted_on,'%Y-%m-%d') as `deleted on`";
  $fieldNameStrOrder = "dfg_id,dfg_name,dfg_remarks,`status`,`is deleted`,`company`,`created by`,`created on`,`modified by`,`last modified on`,`deleted by`,`deleted on`";
  $condition[] = "dfg_id like '%".$search['value']."%'";
  $condition[] = "dfg_name like '%".$search['value']."%'";
  $condition[] = "dfg_remarks like '%".$search['value']."%'";
  $condition[] = "stat_name like '%".$search['value']."%'";
  $condition[] = "if(dfg_is_deleted='1','Yes','No') like '%".$search['value']."%'";
  $condition[] = "syc_name like '%".$search['value']."%'";
  $condition[] = "cu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(dfg_created_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "mu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(dfg_last_modified_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "du.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(dfg_deleted_on,'%Y-%m-%d') = '".$search['value']."'";
  
  $conditionColumnSearch[] = "dfg_id like '%@@value@@%'";
  $conditionColumnSearch[] = "dfg_name like '%@@value@@%'";
  $conditionColumnSearch[] = "dfg_remarks like '%@@value@@%'";
  $conditionColumnSearch[] = "stat_name like '%@@value@@%'";
  $conditionColumnSearch[] = "if(dfg_is_deleted='1','Yes','No') like '%@@value@@%'";
  $conditionColumnSearch[] = "syc_name like '%@@value@@%'";
  $conditionColumnSearch[] = "cu.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(dfg_created_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "mu.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(dfg_last_modified_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "du.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(dfg_deleted_on,'%Y-%m-%d') = '@@value@@'";
  ...
  $sql = "select ".$fieldNameStr."
        from dms_file_group
          left join sys_status on dfg_status=stat_id
          left join sys_companies on dfg_company_id=syc_id
          left join sys_users cu on dfg_created_by= cu.syu_id
          left join sys_users mu on dfg_last_modified_by= mu.syu_id
          left join sys_users du on dfg_deleted_by= du.syu_id
