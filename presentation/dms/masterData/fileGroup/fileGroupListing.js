...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#dms_file_groupToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#griddms_file_group tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#griddms_file_group').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./fileGroupListing-db-get.php",
  ...
  $('#griddms_file_group').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmdms_file_groupListing #btnClose").click(function(){  
