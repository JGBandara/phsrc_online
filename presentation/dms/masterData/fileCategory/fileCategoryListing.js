...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#dms_file_categoryToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#griddms_file_category tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#griddms_file_category').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./fileCategoryListing-db-get.php",
  ...
  $('#griddms_file_category').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmdms_file_categoryListing #btnClose").click(function(){  
