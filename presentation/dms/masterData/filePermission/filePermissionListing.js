...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#dms_file_permissionToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#griddms_file_permission tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#griddms_file_permission').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./filePermissionListing-db-get.php",
  ...
  $('#griddms_file_permission').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmdms_file_permissionListing #btnClose").click(function(){  
