...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#dms_trn_file_access_logToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#griddms_trn_file_access_log tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#griddms_trn_file_access_log').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./fileAccessLogListing-db-get.php",
  ...
  $('#griddms_trn_file_access_log').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmdms_trn_file_access_logListing #btnClose").click(function(){  
