    ...
    if($row = mysqli_fetch_row($result)){
     ...
     $sql="update `institute_payment_detail`
      ...
      $sql="insert into `institute_payment_detail`
            ( payment_detail_institute_id,payment_reg_year,payment_reg_fee,payment_stamp_fee,payment_amount,payment_arrears,payment_date,payment_branch,payment_type,payment_reg_type_id, payment_detail_company_id, payment_detail_created_by, payment_detail_created_on)
              values 
                ('$id','$txtYear','$txtRegFee','$txtStampFee','$txtAmount','$txtArrears','$txtPaymentDate','$txtPaymentBranch','$paymentType','1','$userCompanyId', '$userCompanyId', now())";
...
//    $clsApprove->newApprovalEntry($autoNoType, $entryId, $noReference, true);
    if($finalResult){                    
        $response['type'] 	= 'pass';
        $response['msg'] 	= 'Your application was saved successfully.(if you not using online payment you must attach the payment slip ) ';
...
elseif($requestType=='resubmit'){
  try{
    ...
    if($finalResult){                    
        $response['type'] 	= 'pass';
        $response['msg'] 	= 'Your application was updated successfully';
        $response['id'] 	= $entryId;
        $db->commit();
    }
    else{                    
...
//$newImgName=$imgName;
  if ($size <=10 && ($fType == "image/gif" || $fType == "image/jpeg" || $fType == "image/jpg" || $fType == "image/pjpeg" || $fType == "image/x-png" || $fType == "image/png")){
    if ($file["error"] > 0){
      ...
      $target2 = $backwardSeparator."img/BankSlip/40" . $newImgName;
      $target3 = $backwardSeparator."img/BankSlip/32/" . $newImgName;
