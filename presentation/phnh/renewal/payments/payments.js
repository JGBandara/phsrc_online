    ...
    var httpobj = $.ajax({
        ...
        success:function(json){
          ...
          if(json){ 
			  ...
			  //alert(imageName)
			   $('#frm_payment_information .avatar1-pic').attr('src',backwardSeparator+'img/BankSlip/fpds/'+imageName); 
...
function loadSearchCombo(searchId){
	
	var url = "misInformation-db-get.php?requestType=loadSearchCombo&id="+searchId;
	var httpobj = $.ajax({url:url,async:false})
	$('#frm_payment_information #cboSearch').html(httpobj.responseText);
	$('#frm_payment_information #cboSearch').val(searchId);
	$('#frm_payment_information #cboSearch').trigger('change');
}
