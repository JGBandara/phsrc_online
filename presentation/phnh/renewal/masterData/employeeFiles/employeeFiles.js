...
$(document).ready(function () {
  ...
  $("#frmhrm_employee_files #btnSave").click(function () {
    var requestType = "";
    var id = "";
    if ($("#frmhrm_employee_files").valid()) {
      // test for validity
      //------------------------------------------
      // upload files
      //------------------------------------------
  ...
  var divDropzone = new Dropzone("#abc-efg", {
    ...
    url:
      backwardSeparator +
      "presentation/dms/file/file-db-set.php?requestType=upload",
    ...
    init: function () {
      ...
      this.on("success", function (file, response) {
        ...
        var idInput = Dropzone.createElement(
          '<input type="hidden" class="doc_id" value="' + cd.id + '"/>'
        );
        var urlInput = Dropzone.createElement(
          '<input type="hidden" class="doc_url" value="' + cd.url + '"/>'
        );
      ...
      this.on("addedfile", function (file) {
        var removeButton = Dropzone.createElement(
          '<div class="filePreviewDeleteButton"><a href="#"><img src="' +
            backwardSeparator +
            'img/core/deletered.png"></img></a></div>'
        );
        var _this = this;
        removeButton.addEventListener("click", function (e) {
          ...
          $.ajax({
            ...
            url:
              backwardSeparator +
              "presentation/dms/file/file-db-set.php?requestType=unlink",
            data: name,
            dataType: "json",
          });
        });
        file.previewElement.append(removeButton);
        var remarksButton = Dropzone.createElement(
          '<div class="filePreviewUpdateButton" id="divPreviewBtn_' +
            index +
            '"><a href="#"><img src="' +
            backwardSeparator +
            'img/core/edit.png"></img></a></div>'
        );
        var _this = this;
...
function loadFileList($id) {
  ...
  var httpobj = $.ajax({
    ...
    success: function (json) {
      if (json) {
        for (var i = 0; i < json.length; i++) {
          $("#tblEmpExistingDocuments1").append(
            '<tr class="cloneRow" style=""><td class="category p-1">' +
              json[i].dfc_name +
              '</td><td class="name p-1">' +
              json[i].dfc_is_mandatory +
              "</td></tr>"
          );
        }
      }
