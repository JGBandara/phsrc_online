...
$( document ).ready( function () {
  ...
  //      Save Function
  // --------------------------------------------------------
  if($('#frm_institute_facility #cboSearch').val()==''||$('#frm_institute_facility #cboSearch').val()==null){
	 $('#frm_institute_facility #btnSave').hide();
	  }else{ 
	  $('#frm_institute_facility #btnSave').show();
	  }
  ...
  //---------------tblMainGrid1---------------------------
    ...
    if($("#frm_institute_facility").valid()){   // test for validity
      ...
      var obj = $.ajax({
			...
			dataType: "json",
			type:'post', 
			data:'requestType='+requestType+'&cboSearch='+id+'&facilityDetail='+value,
			async:false,
...
function loadSearchCombo($id){
	var url = "instituteFacility-db-get.php?requestType=loadSearchCombo";
