...
if($requestType=='loadDetails'){
  $id = $_REQUEST['id'];
   $sql = "select dfi_id, dfi_file_name, dfi_file_extension, dfi_store_location, dfi_url, dfi_reference_no, dfi_reference_id, 
        ...
        where 1=1 and dfi_company_id='$userCompanyId' and dfi_reference_id='$id' and dfg_id='13' and dfi_is_deleted='0'
...
Inner Join man_institute_main ON institute_registration.ins_type_id = man_institute_main.main_cat_id 
 where  institute_payment_detail.paymet_is_success=1 and institute_registration.ins_province_id=$userLocationId and institute_payment_detail.Inspection_user=$userId order by ins_application_id asc";
		  $html = "<option vlaue=\"\"></option>";
	$result=$db->singleQuery($sql);
	while($row=mysqli_fetch_array($result)){
		
		$html .= "<option value=\"".$row['ins_application_id']."\">".$row['ins_institute_name']." ".$row['reg_no']."</option>";
