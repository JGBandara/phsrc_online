...
if($requestType=='approve'){
  ...
  try{
    ...
    $sql="update `ppds_new_registration`
          set
            ppds_pd_approval = '1',
            ppds_pd_approved_on =now(),
            ppds_pd_approved_by = '$userId'
          where ppds_application_id='$id' ";
...
tbl_owner.ownership,
ppds_payment_detail.payment_reg_fee,
ppds_payment_detail.payment_stamp_fee
FROM
...
";
	...
	while($row=mysqli_fetch_array($result)){
		 $applicationId=$row['ppds_application_id'];
		$ownerName=$row['ppds_owner_name'];
		$relationsip=$row['ppds_owner_relationship'];
		$owAddress=$row['ppds_owner_address'];
		$insName=$row['ppds_institute_name'];
		$insAddress=$row['ppds_institute_address'];
		$telNo='';
		$fax='';
		$email='';
		$web='';
		$date='2019-10-10';
		$description='';
		$provinceId= $row['ppds_province_id'];
		$distrctId= $row['ppds_district_id'];
		$approvalStatus=$row['ppds_pd_approval'];
		//------------------------------------------------------------------------------------------------
	...
	where is_deleted='0' and reg_no  like '%PHSRC/PDS%'";
