...
if($requestType=='approve'){
  ...
  try{
    ...
    $sql="update `fpds_new_registration`
          set
            fpds_pd_approval = '1',
            fpds_pd_approved_on =now(),
            fpds_pd_approved_by = '$userId'
          where fpds_application_id='$id' ";
...
tbl_owner.ownership,
fpds_payment_detail.payment_reg_fee,
fpds_payment_detail.payment_stamp_fee
FROM
...
";
	...
	while($row=mysqli_fetch_array($result)){
		...
		$date='2019-10-10';
		$description='';
		$provinceId= $row['fpds_province_id'];
		$distrctId= $row['fpds_district_id'];
