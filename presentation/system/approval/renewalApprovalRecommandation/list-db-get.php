...
if ($requestType == 'loadDetails') {
  ...
  where sys_users.syu_id='$userId' and sys_user_location.syo_is_deleted='0' and payment_is_approval='10' and Inspection_user=$userId order by ins_application_id asc";
	//  where institute_registration.ins_province_id=$userLocationId
	$result = $db->singleQuery($sql);
	while ($row = mysqli_fetch_array($result)) {

		$response['id'][] = $row['ins_application_id'];
		$response['ownerName'][] = $row['reg_no'];
		$response['crDate'][] = $row['payment_date'];
		$response['paymentType'][] = $row['ins_institute_name'];
