...
if($requestType=='approve'){
  ...
  try{
    ...
    $sql="update `fpdm_new_registration`
          set
            fpdm_pd_approval = '1',
            fpdm_pd_approved_on =now(),
            fpdm_pd_approved_by = '$userId'
          where fpdm_application_id='$id' ";
...
tbl_owner.ownership,
fpdm_payment_detail.payment_reg_fee,
fpdm_payment_detail.payment_stamp_fee
FROM
...
";
	...
	while($row=mysqli_fetch_array($result)){
		 $applicationId=$row['fpdm_application_id'];
		$ownerName=$row['fpdm_owner_name'];
		$relationsip=$row['fpdm_owner_relationship'];
		$owAddress=$row['fpdm_owner_address'];
		$insName=$row['fpdm_institute_name'];
		$insAddress=$row['fpdm_institute_address'];
		$telNo='';
		$fax='';
		$email='';
		$web='';
		$date='2019-10-10';
		$description='';
		$provinceId= $row['fpdm_province_id'];
		$distrctId= $row['fpdm_district_id'];
		$approvalStatus=$row['fpdm_pd_approval'];
		//------------------------------------------------------------------------------------------------
	...
	where is_deleted='0' and reg_no  like '%PHSRC/FGP%'";
	...
	while($row=mysqli_fetch_array($result))
		{
		 ...
		 $newRegNo='PHSRC/FGP/'.++$serial;
	
 $sql="INSERT INTO reg_ins_comm (mainCat_id, subCat_id, ins_name, reg_no, address, telephone, fax, email, web, Start_date, discription, is_slmc_mem, slmc_no, is_display_web, is_close, is_board_issued, is_bio_reg, bis_registration, hours_of_prctices, province_id, district_id, ownership_type, record_keeping, price_cat_id, annual_fee,created_by,created_date,is_deleted) VALUES ('6','0','$insName','$newRegNo','$address','$telNo','$fax','$email','$web','$date','$description',1,'$slmc',$intDisplay,$intClouse,$intBo,$intBio,'$bisRegistration','$prcticeHR',$provinceId,$distrctId,$ownrShip,$recordKeep,6,$annalFee,'0',now(),0)";
		$result =mysqli_query($conn,$sql);
