...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#sys_trn_loginToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridsys_trn_login tfoot th').each( function ($k) {
      if($k>=1){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridsys_trn_login').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./loginListing-db-get.php",
  ...
  $('#gridsys_trn_login').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmsys_trn_loginListing #btnClose").click(function(){  
