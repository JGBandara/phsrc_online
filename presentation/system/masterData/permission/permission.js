...
//  $('#frmsys_permission #btnReject').hide();
  $('#frmsys_permission #cboUser').prop('disabled', true);
  $('#frmsys_permission #cboModule').prop('disabled', true);
  
  if(intViewx){ // View Permission
 	$('#frmsys_permission #cboUser').prop('disabled', false);
...
//  }
  ...
  $("#frmsys_permission #btnDelete").click(function(){  
    ...
    if($('#frmsys_permission #cboUser').val()==''){
      ...
      id = $('#frmsys_permission #cboUser').val();
    }
    var url = "permission-db-set.php";
    var obj = $.ajax({
          url:url,
          dataType: "json",
          type:'post', 
          data:'requestType='+requestType+'&grant_user_id='+id,
          async:false,
  ...
  // ===============  Load Access Grid =========
  // ===========================================
  $("#frmsys_permission #cboUser").change(function(){  
    loadPermissionGrid();
  });
...
function loadPermissionGrid(){
  ...
  var httpobj = $.ajax({
      url:url,
      dataType:'json',
      data:'requestType=loadPermissionGrid&userId='+$userId+'&module='+$moduleName,
      async:false,
      success:function(json){
        $('#frmsys_permission #tBodyPermission .dataRow').each(function(){
          $(this).remove();
...
function menuPermission(event){
  ...
  var obj = $.ajax({
        ...
        dataType: "json",
        type:'post', 
        data:'requestType='+requestType+'&grant_user_id='+$userId+'&menu_id='+$menuId+'&status='+$status+$para,
        async:false,
