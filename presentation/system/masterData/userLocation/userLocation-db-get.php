...
if($requestType=='loadDetails'){
  $id = $_REQUEST['id'];
  $sql = "select syo_location_id
              from sys_user_location 
              where syo_company_id='$userCompanyId' and syo_is_deleted='0' and syo_status='1' and syo_user_id='$id'";
  $result = $db->singleQuery($sql);
  $arr = [];
  while($row = mysqli_fetch_array($result)){
    $arr[] = $row['syo_location_id'];
  }
  $response['ids']=$arr;
...
//    $model->syo_id = $recordId;
//    $model->syo_company_id = $userCompanyId;
