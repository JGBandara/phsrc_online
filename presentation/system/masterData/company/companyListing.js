...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#sys_companiesToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridsys_companies tfoot th').each( function ($k) {
      if($k!=0){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridsys_companies').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./companyListing-db-get.php",
  ...
  //      Close Function
  // ---------------------------------
  $("#frmsys_companiesListing #btnClose").click(function(){  
