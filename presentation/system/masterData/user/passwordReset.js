...
$( document ).ready( function () {
  ...
  // =====================================================
  $("#frmsys_reset_users #cboSearch").focus(function(){  
    loadSearchCombo($(this).val());
  });
  // ========================================
  // ===============  Search Record =========
  // ========================================
  $("#frmsys_reset_users #cboSearch").change(function(){  
    $("#frmsys_reset_users").validate().resetForm();
    var url = "user-db-get.php";
    if($('#frmsys_reset_users #cboSearch').val()==''){
        $('#frmsys_reset_users').get(0).reset();
        $('#frmsys_reset_users #lblName').html('');
        return;	
    }
    var httpobj = $.ajax({
        url:url,
        dataType:'json',
        data:'requestType=randomPassword&id='+$(this).val(),
        async:false,
        success:function(json){
          $('#frmsys_reset_users #txtPassword').val(json.password);
          $('#frmsys_reset_users #lblName').html(json.name);
        }
    });
  });
...
function loadSearchCombo($id){
	...
	$('#frmsys_reset_users #cboSearch').html(httpobj.responseText);
	$('#frmsys_reset_users #cboSearch').val($id);
	$('#frmsys_reset_users #cboSearch').trigger('change');
}
