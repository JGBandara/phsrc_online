...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#sys_alert_categoryToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridsys_alert_category tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridsys_alert_category').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./alertCategoryListing-db-get.php",
  ...
  $('#gridsys_alert_category').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmsys_alert_categoryListing #btnClose").click(function(){  
