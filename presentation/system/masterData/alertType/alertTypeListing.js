...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#sys_alert_typeToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridsys_alert_type tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridsys_alert_type').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./alertTypeListing-db-get.php",
  ...
  $('#gridsys_alert_type').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmsys_alert_typeListing #btnClose").click(function(){  
