...
$('#cboPayType').change(function(){
  ...
  $("#frm_payment_information #btnResubmit").click(function(){
    var requestType = 'resubmit';
    var id = '';
    if($('#frm_payment_information #cboSearch').val()==''){
      modalMsgBox("Error", "Record is not selected ...");	
      return false;
             ...
             $('#frm_payment_information #txtPaymentDate').val(json.paymentDate);
			  ...
			  //alert(imageName)
			   $('#frm_payment_information .avatar1-pic').attr('src',backwardSeparator+'img/BankSlip/'+imageName); 
                         ...
                         $('#frm_payment_information #txtStampFee').val(1500);
...
function loadSearchCombo($id){
	$("#msgPayment").text("");
	var url = "payments-db-get.php?requestType=loadSearchCombo";
...
function calculate(){
       ...
       $addAmount= parseFloat($('#frm_payment_information #txtArrears').val(20000));
