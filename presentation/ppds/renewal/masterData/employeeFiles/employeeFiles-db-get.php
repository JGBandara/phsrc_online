 ...
 $id = $_REQUEST['id'];
  ...
  $sql = "select payment_is_approval,dfi_id, dfi_file_name, dfi_file_extension, dfi_store_location, dfi_url, dfi_reference_no, dfi_reference_id, 
       ...
       /*   left join dms_file_permission on dfp_file_category_id=dfi_file_category_id and dfp_user_id='$userId' and dfp_status='1' and dfp_is_deleted='0'*/
        LEFT JOIN institute_payment_detail ON dfi_reference_id = payment_detail_institute_id 
        where 1=1 and dfi_company_id='$userCompanyId' and dfi_reference_id='$id' and dfg_id='8' and dfi_is_deleted='0'
...
) and dms_file_category.dfc_file_group_id=8 and dms_file_category.dfc_status=1 and dms_file_category.dfc_is_deleted=0";
  $result = $db->singleQuery($sql);
  $arr = [];
  while($row = mysqli_fetch_assoc($result)){
    $arr[] = $row;
...
reg_no
FROM
institute_registration where ins_is_deleted='0' and ins_type_id='8'  and institute_reg_id='$id'
          order by ins_application_id asc";
		 
	$result=$db->singleQuery($sql);
	while($row=mysqli_fetch_array($result)){
		
		$html .= "<option value=\"".$row['ins_application_id']."\">".$row['reg_no']."</option>";
