...
$txtOfficeAddress     = isset($_REQUEST['txtOfficeAddress'])?trim($_REQUEST['txtOfficeAddress']):null;
...
$txtTelephone           = isset($_REQUEST['txtTelephone'])?trim($_REQUEST['txtTelephone']):null;
$txtMobile           = isset($_REQUEST['txtMobile'])?trim($_REQUEST['txtMobile']):null;
$txtEmail           = isset($_REQUEST['txtEmail'])?trim($_REQUEST['txtEmail']):null;
$txtWebsite           = isset($_REQUEST['txtWebsite'])?trim($_REQUEST['txtWebsite']):null;
...
//      $noReference	= $clsAutoNo->encodeNo($autoNo,"Em");
    ...
    $sql="insert into `institute_registration`(ins_type_id,ins_owner_name,ins_owner_relationship,ins_owner_offic_address,ins_owner_address,ins_institute_name,ins_institute_address,ins_telephone,ins_mobile,ins_email,ins_website,ins_province_id,ins_district_id,ins_company_id,ins_created_by,ins_created_on)
              values                ('$insTypeId','$txtName','$txtRelIns','$txtOfficeAddress','$txtAddress','$txtInsName','$txtInsAddress','$txtTelephone','$txtMobile','$txtEmail','$txtWebsite','$cboProvince','$cboDistrict','$userCompanyId','$userId','".time()."')";  
...
elseif($requestType=='edit'){
  try{
    ...
    if($row = mysqli_fetch_array($result)){
        ...
        $referenceId=$row['institute_reg_id'];
   ...
   $sql="update `institute_registration`
          set
            ...
            ins_owner_offic_address='$txtOfficeAddress',
            ins_owner_address='$txtAddress',
            ins_institute_name='$txtInsName',
            ins_institute_address='$txtInsAddress',
            ins_telephone='$txtTelephone',
            ins_mobile='$txtMobile',
            ins_email='$txtEmail',
            ins_website='$txtWebsite',
    ...
    $entryId = $id;           
        ...
        $sql="insert into `institute_registration`(reg_no,institute_reg_id,ins_type_id,ins_owner_name,ins_owner_relationship,ins_owner_offic_address,ins_owner_address,ins_institute_name,ins_institute_address,ins_telephone,ins_mobile,ins_email,ins_website,ins_province_id,ins_district_id,ins_company_id,ins_created_by,ins_created_on)
              values                ('$regNo',$id,'$insTypeId','$txtName','$txtRelIns','$txtOfficeAddress','$txtAddress','$txtInsName','$txtInsAddress','$txtTelephone','$txtMobile','$txtEmail','$txtWebsite','$cboProvince','$cboDistrict','$userCompanyId','$userId','".time()."')";  
        $finalResult = $db->batchQuery($sql);
    }          
...
function saveFile($file,$id){
  ...
  if ($size <=10 && ($fType == "image/gif" || $fType == "image/jpeg" || $fType == "image/jpg" || $fType == "image/pjpeg" || $fType == "image/x-png" || $fType == "image/png")){
    if ($file["error"] > 0){
      ...
      $target1 = $backwardSeparator."drive/insProfile/" . $newImgName;
