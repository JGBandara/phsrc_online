...
}elseif ($requestType=='loadDocList') {
     $id = $_REQUEST['id'];
     $sql = "select ins_application_id from institute_registration where institute_reg_id='$id' ";
    $result = $db->singleQuery($sql);
    while($row= mysqli_fetch_array($result)){
...
";
   ...
   $html = "<option vlaue=\"\"></option>";
	...
	while($row=mysqli_fetch_array($result)){
		...
		$html .= "<option value=\"".$row['dfc_id']."\">".$row['dfc_name']."</option>";
...
where ins_info_institute_id=$id";
  ...
  while($row=  mysqli_fetch_array($result)){
      ...
      if($insTypeId==3){
          ...
          $where="and dms_file_category.dfc_id in(14,15)";
      }elseif($insTypeId==6){
          $where="and dms_file_category.dfc_id in(15,35,36)";
      }
  }
...
) and dms_file_category.dfc_file_group_id=12 and dms_file_category.dfc_status=1 and dms_file_category.dfc_is_deleted=0 $where";
  $result = $db->singleQuery($sql);
  $arr = [];
  while($row = mysqli_fetch_assoc($result)){
    $arr[] = $row;
