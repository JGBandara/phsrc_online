...
if($requestType=='loadDetails'){
  ...
  $sql = "select payment_is_approval,dfi_id, dfi_file_name, dfi_file_extension, dfi_store_location, dfi_url, dfi_reference_no, dfi_reference_id, 
        ...
        where 1=1 and dfi_company_id='$userCompanyId' and dfi_reference_id='$id' and dfg_id='9' and dfi_is_deleted='0'
...
) and dms_file_category.dfc_file_group_id=9 and dms_file_category.dfc_status=1 and dms_file_category.dfc_is_deleted=0";
  $result = $db->singleQuery($sql);
  $arr = [];
  while($row = mysqli_fetch_assoc($result)){
    $arr[] = $row;
