...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#hrm_trn_membershipToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridhrm_trn_membership tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // ---------------------------------
  var table = $('#gridhrm_trn_membership').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./membershipListing-db-get.php",
  ...
  $('#gridhrm_trn_membership').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmhrm_trn_membershipListing #btnClose").click(function(){  
