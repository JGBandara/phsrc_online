...
$conditionStr = "1=1 and eaq_company_id='$userCompanyId' and eaq_location_id='$userLocationId'"; 
  $sql = "select *
  ...
  $fieldNameStr = " eaq_id, emi_no, emi_calling_name, aqt_name, aqs_name, eaq_institute, eaq_year, eaq_index_no, eaq_remarks, ifnull(stat_name,'') as `status`, if(eaq_is_deleted='1','Yes','No') as `is deleted`, ifnull(syl_name,'') as `location`, ifnull(syc_name,'') as `company`, ifnull(cu.syu_full_name,'') as `created by`, from_unixtime(eaq_created_on,'%Y-%m-%d') as `created on`, ifnull(mu.syu_full_name,'') as `modified by`, from_unixtime(eaq_last_modified_on,'%Y-%m-%d') as `last modified on`, ifnull(du.syu_full_name,'') as `deleted by`, from_unixtime(eaq_deleted_on,'%Y-%m-%d') as `deleted on`";
  $fieldNameStrOrder = "eaq_id,emi_no,emi_calling_name,aqt_name,aqs_name,eaq_institute,eaq_year,eaq_index_no,eaq_remarks,`status`,`is deleted`,`location`,`company`,`created by`,`created on`,`modified by`,`last modified on`,`deleted by`,`deleted on`";
  $condition[] = "eaq_id like '%".$search['value']."%'";
  $condition[] = "emi_no like '%".$search['value']."%'";
  $condition[] = "emi_calling_name like '%".$search['value']."%'";
  $condition[] = "aqt_name like '%".$search['value']."%'";
  $condition[] = "aqs_name like '%".$search['value']."%'";
  $condition[] = "eaq_institute like '%".$search['value']."%'";
  $condition[] = "eaq_year like '%".$search['value']."%'";
  $condition[] = "eaq_index_no like '%".$search['value']."%'";
  $condition[] = "eaq_remarks like '%".$search['value']."%'";
  $condition[] = "stat_name like '%".$search['value']."%'";
  $condition[] = "if(eaq_is_deleted='1','Yes','No') like '%".$search['value']."%'";
  $condition[] = "syl_name like '%".$search['value']."%'";
  $condition[] = "syc_name like '%".$search['value']."%'";
  $condition[] = "cu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(eaq_created_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "mu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(eaq_last_modified_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "du.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(eaq_deleted_on,'%Y-%m-%d') = '".$search['value']."'";
  
  $conditionColumnSearch[] = "eaq_id like '%@@value@@%'";
  ...
  $conditionColumnSearch[] = "aqs_name like '%@@value@@%'";
  $conditionColumnSearch[] = "eaq_institute like '%@@value@@%'";
  $conditionColumnSearch[] = "eaq_year like '%@@value@@%'";
  $conditionColumnSearch[] = "eaq_index_no like '%@@value@@%'";
  $conditionColumnSearch[] = "eaq_remarks like '%@@value@@%'";
  $conditionColumnSearch[] = "stat_name like '%@@value@@%'";
  $conditionColumnSearch[] = "if(eaq_is_deleted='1','Yes','No') like '%@@value@@%'";
  ...
  $conditionColumnSearch[] = "from_unixtime(eaq_created_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "mu.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(eaq_last_modified_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "du.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(eaq_deleted_on,'%Y-%m-%d') = '@@value@@'";
  ...
  $sql = "select ".$fieldNameStr."
        from hrm_trn_academic_qualification
          ...
          left join sys_status on eaq_status=stat_id
          left join sys_location on eaq_location_id=syl_id
          left join sys_companies on eaq_company_id=syc_id
          left join sys_users cu on eaq_created_by= cu.syu_id
          left join sys_users mu on eaq_last_modified_by= mu.syu_id
          left join sys_users du on eaq_deleted_by= du.syu_id
