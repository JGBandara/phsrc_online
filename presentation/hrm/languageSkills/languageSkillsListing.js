...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#hrm_trn_language_skillsToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridhrm_trn_language_skills tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
...
//  alert(navigator.userAgent.indexOf('Firefox')+'::'+$fixedColumns);
  var table = $('#gridhrm_trn_language_skills').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./languageSkillsListing-db-get.php",
  ...
  $('#gridhrm_trn_language_skills').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmhrm_trn_language_skillsListing #btnClose").click(function(){  
