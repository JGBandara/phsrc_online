...
$conditionStr = "1=1 and aqt_company_id='$userCompanyId'"; 
  $sql = "select *
  ...
  $fieldNameStr = " aqt_id, aqt_name, aqt_remarks, ifnull(stat_name,'') as `status`, if(aqt_is_deleted='1','Yes','No') as `is deleted`, ifnull(syc_name,'') as `company`, ifnull(cu.syu_full_name,'') as `created by`, from_unixtime(aqt_created_on,'%Y-%m-%d') as `created on`, ifnull(mu.syu_full_name,'') as `modified by`, from_unixtime(aqt_last_modified_on,'%Y-%m-%d') as `last modified on`, ifnull(du.syu_full_name,'') as `deleted by`, from_unixtime(aqt_deleted_on,'%Y-%m-%d') as `deleted on`";
  $fieldNameStrOrder = "aqt_id,aqt_name,aqt_remarks,`status`,`is deleted`,`company`,`created by`,`created on`,`modified by`,`last modified on`,`deleted by`,`deleted on`";
  $condition[] = "aqt_id like '%".$search['value']."%'";
  $condition[] = "aqt_name like '%".$search['value']."%'";
  $condition[] = "aqt_remarks like '%".$search['value']."%'";
  $condition[] = "stat_name like '%".$search['value']."%'";
  $condition[] = "if(aqt_is_deleted='1','Yes','No') like '%".$search['value']."%'";
  $condition[] = "syc_name like '%".$search['value']."%'";
  $condition[] = "cu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(aqt_created_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "mu.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(aqt_last_modified_on,'%Y-%m-%d') = '".$search['value']."'";
  $condition[] = "du.syu_full_name like '%".$search['value']."%'";
  $condition[] = "from_unixtime(aqt_deleted_on,'%Y-%m-%d') = '".$search['value']."'";
  
  $conditionColumnSearch[] = "aqt_id like '%@@value@@%'";
  $conditionColumnSearch[] = "aqt_name like '%@@value@@%'";
  $conditionColumnSearch[] = "aqt_remarks like '%@@value@@%'";
  $conditionColumnSearch[] = "stat_name like '%@@value@@%'";
  $conditionColumnSearch[] = "if(aqt_is_deleted='1','Yes','No') like '%@@value@@%'";
  $conditionColumnSearch[] = "syc_name like '%@@value@@%'";
  $conditionColumnSearch[] = "cu.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(aqt_created_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "mu.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(aqt_last_modified_on,'%Y-%m-%d') = '@@value@@'";
  $conditionColumnSearch[] = "du.syu_full_name like '%@@value@@%'";
  $conditionColumnSearch[] = "from_unixtime(aqt_deleted_on,'%Y-%m-%d') = '@@value@@'";
  ...
  $sql = "select ".$fieldNameStr."
        from hrm_academic_qualification_type
          left join sys_status on aqt_status=stat_id
          left join sys_companies on aqt_company_id=syc_id
          left join sys_users cu on aqt_created_by= cu.syu_id
          left join sys_users mu on aqt_last_modified_by= mu.syu_id
          left join sys_users du on aqt_deleted_by= du.syu_id
