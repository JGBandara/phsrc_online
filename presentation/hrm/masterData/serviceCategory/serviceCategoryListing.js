...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#hrm_service_categoryToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridhrm_service_category tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridhrm_service_category').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./serviceCategoryListing-db-get.php",
  ...
  $('#gridhrm_service_category').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmhrm_service_categoryListing #btnClose").click(function(){  
