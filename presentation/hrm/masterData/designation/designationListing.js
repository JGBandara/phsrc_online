...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#hrm_designationToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridhrm_designation tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // Create Table
  // ---------------------------------
  var table = $('#gridhrm_designation').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./designationListing-db-get.php",
  ...
  $('#gridhrm_designation').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmhrm_designationListing #btnClose").click(function(){  
