...
$( document ).ready( function () {
  ...
  $buttons = [{text: 'Show/Hide',
      action: function ( e, dt, node, conf ) {
                    $('#hrm_salary_scaleToggleModal').modal();
                },
...
//                },    
  ...
  // Setup - add a text input to each footer cell
    $('#gridhrm_salary_scale tfoot th').each( function ($k) {
      if($k>=2){
        var title = $(this).text();
  ...
  // ---------------------------------
  var table = $('#gridhrm_salary_scale').DataTable({
    processing: true,
    serverSide: true,
    ajax: "./salaryScaleListing-db-get.php",
  ...
  $('#gridhrm_salary_scale').on('draw.dt', function () {
  ...
  //      Close Function
  // ---------------------------------
  $("#frmhrm_salary_scaleListing #btnClose").click(function(){  
